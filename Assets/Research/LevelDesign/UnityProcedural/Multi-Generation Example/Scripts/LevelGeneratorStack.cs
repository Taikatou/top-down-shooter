...
namespace Research.LevelDesign.UnityProcedural.Scripts
{
	public class LevelGeneratorStack : MonoBehaviour
	{
		...
		[SerializeField] public List<MapSettings> mapSettings = new List<MapSettings>();
			...
			for (int i = 0; i < mapSettings.Count; i++)
			{
				...
				switch (mapSettings[i].algorithm)
				{
					...
					case Algorithm.CellularAutomataVonNeuman:
						...
						map = MapFunctions.GenerateCellularAutomata(width, height, seed, mapSettings[i].fillAmount,
							mapSettings[i].edgesAreWalls);
						//Next smooth out the array using the von neumann rules
						map = MapFunctions.SmoothVNCellularAutomata(map, mapSettings[i].edgesAreWalls,
							mapSettings[i].smoothAmount);
						break;
					case Algorithm.CellularAutomataMoore:
						...
						map = MapFunctions.SmoothMooreCellularAutomata(map, mapSettings[i].edgesAreWalls,
							mapSettings[i].smoothAmount);
						break;
					case Algorithm.DirectionalTunnel:
						...
						map = MapFunctions.DirectionalTunnel(map, mapSettings[i].minPathWidth,
